...
export const getProduct =
  ...
  async (dispatch) => {
    try {
      ...
      //  &page=${currentPage}&price[gte]=${price[0]}&price[lte]=${price[1]}&ratings[gte]=${ratings}`;
...
export const createProduct = (productData) => async (dispatch) => {
  try {
    ...
    const { data } = await apiInstance.post("product/new", productData,config);
...
export const deleteProduct = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await apiInstance.delete(`product/${id}`);
...
export const newReview = (reviewData) => async (dispatch) => {
  try {
    ...
    const { data } = await apiInstance.put('review', reviewData);
